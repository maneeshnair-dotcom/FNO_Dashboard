...
FnO = [
    ...
    "LODHA.NS", "LT.NS", "LTF.NS", "LTM.NS", "LUPIN.NS", "M&M.NS", "MAHABANK.NS","MANAPPURAM.NS", "MANKIND.NS",
...
# ── Main Function ─────────────────────────────────────────────────────────────
def _download_with_retry(tickers, period, interval, max_attempts=4):
    """yf.download() against Yahoo Finance occasionally hits a 429 rate
    limit (very common on shared-IP hosts like Streamlit Cloud — you'll
    see 'Crumb fetch rate-limited' in the logs right before it). That's
    almost always transient, so retry with exponential backoff + jitter
    before giving up, instead of failing the whole app on the first hit."""
    last_err = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if raw is not None and not raw.empty:
                return raw
            last_err = "yfinance returned an empty result (likely rate-limited)"
        except Exception as e:
            last_err = e
        if attempt < max_attempts:
            wait = (2 ** attempt) + random.uniform(0, 1.5)
            print(f"  ⚠ Download attempt {attempt}/{max_attempts} failed ({last_err}); "
                  f"retrying in {wait:.1f}s...")
            time.sleep(wait)
    raise ValueError(
        f"No data fetched after {max_attempts} attempts — Yahoo Finance is likely rate-limiting "
        f"this host right now (last error: {last_err}). This is usually transient; try again in "
        f"a minute or two."
    )
...
def fetch_nifty50_data(n_days: int = 30, interval: str = "1d",
    ...
    raw = _download_with_retry(yf_tickers, period=f"{n_days}d", interval=fetch_interval)
