...
def render_correlation_tab(df: pd.DataFrame, interval: str, key_suffix: str):
    """Pairwise correlation of candle-to-candle % returns across the
    currently selected ticker list, for the currently selected interval."""
    st.markdown('<div class="sec-label">🔗 Correlation Heatmap — how closely do these stocks move together?</div>',
                unsafe_allow_html=True)
    st.caption(f"Pairwise correlation of {interval} candle-to-candle % returns, computed across this "
               f"run's full fetched history. +1 = move together, -1 = move oppositely, 0 = unrelated.")

    if df is None or df.empty or "Date" not in df.columns or "CLOSE" not in df.columns:
        st.info("Not enough data to compute correlations.")
        return

    wide_close = df.pivot_table(index="Date", columns="Stock Name", values="CLOSE", aggfunc="last")
    returns = wide_close.pct_change().dropna(how="all")

    min_obs = max(10, int(len(returns) * 0.5))
    valid_cols = returns.columns[returns.notna().sum() >= min_obs]
    dropped = sorted(set(wide_close.columns) - set(valid_cols))
    returns = returns[valid_cols]

    all_stocks = sorted(returns.columns.tolist())
    if len(all_stocks) < 2:
        st.info("Need at least 2 stocks with enough overlapping history to compute correlation.")
        return

    default_n = min(20, len(all_stocks))
    selected = st.multiselect(
        "Stocks to include (large lists get unreadable — pick a subset, or leave the default)",
        all_stocks, default=all_stocks[:default_n], key=f"corr_select_{key_suffix}",
    )
    if len(selected) < 2:
        st.info("Pick at least 2 stocks to compare.")
        return
    if len(selected) > 60:
        st.warning("60+ stocks selected — the heatmap will be hard to read. Consider narrowing your selection.")
    if dropped:
        with st.expander(f"{len(dropped)} stock(s) excluded — not enough overlapping history"):
    ...
    fig.update_layout(
        height=max(420, 28 * len(selected) + 150), template="plotly_white",
        paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF", font_color="#00355F",
        margin=dict(t=30, b=10, l=10, r=10),
    )
    fig.update_xaxes(tickangle=45)
    st.plotly_chart(fig, width='stretch', key=f"corr_heatmap_{key_suffix}")

    mask = np.triu(np.ones(corr.shape, dtype=bool), k=1)
    pairs = (corr.rename_axis(index="Stock A", columns="Stock B")
                  .where(mask).stack().reset_index())
    pairs.columns = ["Stock A", "Stock B", "Correlation"]
    pairs = pairs.dropna(subset=["Correlation"]).sort_values("Correlation", ascending=False)

    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Most correlated pairs**")
        st.dataframe(pairs.head(10).round(3), width='stretch', hide_index=True,
                     key=f"corr_top_{key_suffix}")
    with c2:
        st.markdown("**Most inversely correlated pairs**")
        st.dataframe(pairs.tail(10).sort_values("Correlation").round(3), width='stretch',
                     hide_index=True, key=f"corr_bottom_{key_suffix}")

    if not pairs.empty:
        st.caption(f"Average pairwise correlation across the selected {len(selected)} stocks: "
...
def render_dashboard_body(df, key_suffix, agree_pos=None, agree_neg=None):
    ...
    lsma_wma_pos_stocks = sorted(latest.loc[latest["LSMA-WMA_Diff"] > 0,  "Stock Name"].tolist())
...
if need_reference_tab and df_1d is not None:
    ...
    tab_primary, tab_ref, tab_corr = st.tabs([
        f"📊 {interval}  (Your Selection)", "📅 1D  (Reference)", "🔗 Correlation",
    ])
    with tab_primary:
    ...
    with tab_corr:
        render_correlation_tab(df, interval, key_suffix="sel")
    ...
    tab_primary, tab_corr = st.tabs([f"📊 {interval}  (Your Selection)", "🔗 Correlation"])
    with tab_primary:
